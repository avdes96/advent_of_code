...
def get_data(filename: str, expanded_grid: bool = False) -> tuple[list[list[str]], str]:
    is_grid = True
    grid = []
    dirs = ""
...
def get_solution(filename: str, expanded_grid: bool = False) -> int:
    grid, directions = get_data(filename, expanded_grid)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Advent of Code 2024 - Day 15")
    parser.add_argument("input_filename", type=str, help="File containing problem input.")
    args = parser.parse_args()
    filename = args.input_filename
    print(f"The answer to part 1 is {get_solution(filename, expanded_grid=False)}.")
    print(f"The answer to part 2 is {get_solution(filename, expanded_grid=True)}.")
