...
def get_data(filename: str) -> tuple[int, int, list[tuple[int, int]]]:
    if "test" in filename:
        height, width = 7, 7
    else:
        height, width = 71, 71
...
def find_shortest_route(grid: list[list[str]], start_x: int, start_y: int, end_x: int, end_y: int) -> int:
    queue = deque()
    ...
    return -1  # Meaning end isn't reachable


def determine_first_square_to_close_path(height: int, width: int, data: list[tuple[int, int]]) -> tuple[int, int]:
...
def part_1(filename: str) -> None:
    height, width, data = get_data(filename)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Advent of Code 2024 - Day 18")
