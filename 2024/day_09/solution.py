...
def perform_swap(block_list: list[int], space: Space, block_start: int, block_end: int) -> list[int]:
    ...
    new_block_list = block_list[:space_start_idx] + [file_id] * block_size + [-1] * \
        remaining_space + block_list[space_start_idx + max(block_size, space_size):]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Advent of Code 2024 - Day 9")
