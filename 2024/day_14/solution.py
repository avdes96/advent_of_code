...
class Grid:
    ...
    def get_var(self) -> float:
        return (variance([robot.position()[0] for robot in self.robots])
                + variance([robot.position()[1] for robot in self.robots])) / 2
...
def get_data(file_name: str) -> Grid:
    ...
    with open(file_name) as file:
        for line in file:
            numbers = findall(r'-?\d+', line.strip())
            x, y, dx, dy = map(int, numbers)
            grid.add_robot(x, y, dx, dy)
    return grid


def display_picture_at_time_t(filename: str, t: int) -> None:
...
def part_2(filename: str) -> None:
    ...
    display_picture_at_time_t(filename, answer)


def main() -> None:
    parser = argparse.ArgumentParser(description="Advent of Code 2024 - Day 14")
