...
def get_data(filename: str) -> defaultdict[str, set[str]]:
...
def bron_kerbosch(graph: defaultdict[str, set[str]]) -> defaultdict[int, list[set]]:
    ...
    def bron_kerbosch_recursive(r: set, p: set, x: set, graph: defaultdict[str, set],
        ...
        for nbr in p.difference(pivot_nbrs):
            bron_kerbosch_recursive(r.union({nbr}), p.intersection(
                graph[nbr]), x.intersection(graph[nbr]), graph, cliques)
            p = p.difference({nbr})
            x = x.union({nbr})
...
def part_1(graph: defaultdict[str, set[str]]) -> None:
...
def part_2(graph: defaultdict[str, set[str]]) -> None:
